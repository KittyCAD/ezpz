...
// Helper to verify arc direction using cross product
// cross > 0 means CCW, cross < 0 means CW
fn verify_arc_direction(
    center: (f64, f64),
    start: (f64, f64),
    end: (f64, f64),
    expected_ccw: bool,
...
fn arc_direction_small_ccw() {
    ...
    let constraints = vec![
        ...
        ConstraintRequest::highest_priority(Constraint::Fixed(start.id_x(), 0.0)),
...
fn arc_direction_large_ccw() {
    ...
    let constraints = vec![
        ConstraintRequest::highest_priority(Constraint::Fixed(center.id_x(), 0.0)),
        ConstraintRequest::highest_priority(Constraint::Fixed(center.id_y(), 0.0)),
        ConstraintRequest::highest_priority(Constraint::Fixed(start.id_x(), -1.0)),
