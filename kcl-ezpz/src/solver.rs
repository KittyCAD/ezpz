...
impl Model {
    ...
    pub(crate) fn is_underconstrained(&mut self) -> Result<bool, crate::NonLinearSystemError> {
        ...
        // Make the symbolic matrix.
        let pairs = gen_pairs(num_cols, num_rows);
        let (sym, _) =
            SymbolicSparseColMat::try_new_from_indices(num_rows, num_cols, &pairs).unwrap();

        let qr_decomposed =
            faer::sparse::linalg::qr::factorize_symbolic_qr(sym.as_ref(), Default::default())
                .map_err(NonLinearSystemError::ValidityCheck)?;

        let answer = false; // TODO
        Ok(answer)
    }
}
